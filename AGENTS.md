...
Read README.md, softrouter/docs/ai-setup.md, and softrouter/docs/compatibility.md before deploying. All agent tools must obey their own permissions and credential handling requirements.

## Scope

- Public code lives under softrouter/. PUBLISH_FILES.txt is the exact release allowlist.
...
- Run /bin/bash softrouter/tests/run.sh after relevant changes. Tests must not alter live PF, routes, DHCP, network services or power settings.
- Add intentional new public files to PUBLISH_FILES.txt. Run python3 softrouter/tools/check-publication.py --staged before committing and --head before packaging.
