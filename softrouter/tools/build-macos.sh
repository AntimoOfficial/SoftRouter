...
if [ "$mode" = --head ]; then
  python3 softrouter/tools/check-publication.py --head
else
...
source="$scratch/source/softrouter"
...
/usr/libexec/PlistBuddy -c "Set :SoftRouterReleaseVersion $version" "$app/Contents/Info.plist"
...
for architecture in arm64 x86_64; do
  xcrun swiftc -swift-version 5 -O -target "$architecture-apple-macosx14.0" \
    ...
    -file-prefix-map "$scratch=/SoftRouterBuild" \
...
chmod 755 "$app/Contents/MacOS/SoftRouter"
...
"$app/Contents/MacOS/SoftRouter" --self-test
python3 - "$scratch/components.plist" <<'PY'
import plistlib,sys
with open(sys.argv[1],'wb') as f:
    plistlib.dump([{'RootRelativeBundlePath':'Applications/SoftRouter.app',
...
if [ ! -e "$preview" ]; then
  mkdir "$preview"
  /usr/bin/ditto "$app" "$preview/SoftRouter.app"
fi
