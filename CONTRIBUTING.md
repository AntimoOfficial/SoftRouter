...
Public additions must be listed in PUBLISH_FILES.txt. Before committing, validate the actual Git index with `python3 air-gateway/tools/check-publication.py --staged`. CI checks the same boundary, but cannot retract material already pushed; local review is required.
