...
INSTALL=/Library/AirGateway
LOGS=/Library/Logs/AirGateway
STATE=/private/var/run/air-gateway
LABEL=org.airgateway.gateway
PLIST=/Library/LaunchDaemons/org.airgateway.gateway.plist
STAGE=
INSTALL_CREATED=0
PLIST_CREATED=0
ENABLED=0
SUCCESS=0
fail() { printf 'AirGateway installation: %s\n' "$*" >&2; exit 1; }
regular_root_file() {
  [ -f "$1" ] && [ ! -L "$1" ] && [ "$(/usr/bin/stat -f '%u:%Lp:%l' "$1")" = "0:$2:1" ]
}
check_payload() {
  local base=$1 p
  for p in gateway.sh gatewayctl config.sh org.airgateway.gateway.plist; do
  ...
  /usr/bin/plutil -lint "$base/org.airgateway.gateway.plist"
  [ "$(/usr/libexec/PlistBuddy -c 'Print :Label' "$base/org.airgateway.gateway.plist")" = "$LABEL" ] || fail 'Wrong launchd label.'
  [ "$(/usr/libexec/PlistBuddy -c 'Print :ProgramArguments:0' "$base/org.airgateway.gateway.plist")" = /bin/bash ] || fail 'Wrong interpreter.'
  [ "$(/usr/libexec/PlistBuddy -c 'Print :ProgramArguments:1' "$base/org.airgateway.gateway.plist")" = "$INSTALL/gateway.sh" ] || fail 'Wrong installed daemon path.'
  [ "$(/usr/libexec/PlistBuddy -c 'Print :ProgramArguments:2' "$base/org.airgateway.gateway.plist")" = --service ] || fail 'Wrong service entry point.'
  [ "$(/usr/libexec/PlistBuddy -c 'Print :StandardOutPath' "$base/org.airgateway.gateway.plist")" = "$LOGS/gateway.log" ] || fail 'Wrong log path.'
  [ "$(/usr/libexec/PlistBuddy -c 'Print :StandardErrorPath' "$base/org.airgateway.gateway.plist")" = "$LOGS/error.log" ] || fail 'Wrong error log path.'
}
...
for p in "$INSTALL" "$PLIST" "$STATE"; do
...
if /bin/launchctl print "system/$LABEL" >/dev/null 2>&1; then fail 'This launchd label is already loaded.'; fi
...
STAGE=$(/usr/bin/mktemp -d /private/tmp/air-gateway-install.XXXXXXXX)
/bin/chmod 700 "$STAGE"
cleanup_install() {
  ...
  if [ "$SUCCESS" = 0 ]; then
    ...
    /bin/rm -f "$STAGE/gateway.sh" "$STAGE/gatewayctl" "$STAGE/config.sh" "$STAGE/gateway.conf" "$STAGE/org.airgateway.gateway.plist" "$STAGE/preflight.log" "$STAGE/pf-before.txt" "$STAGE/prior-status.txt"
...
PLIST_CREATED=1
/usr/bin/install -o root -g wheel -m 644 "$STAGE/org.airgateway.gateway.plist" "$PLIST"
...
SUCCESS=1
printf 'AirGateway installed and RUNNING.\n'
