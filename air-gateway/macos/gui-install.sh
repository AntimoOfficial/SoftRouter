...
export PATH
umask 077
fail() { printf 'Air Gateway: %s\n' "$*" >&2; exit 1; }
valid_digest() { [[ "$1" =~ ^[a-f0-9]{64}$ ]]; }
readonly SOURCE='/Applications/Air Gateway.app/Contents/Resources/core'
...
for directory in '/Applications/Air Gateway.app' '/Applications/Air Gateway.app/Contents' '/Applications/Air Gateway.app/Contents/Resources' "$SOURCE"; do
...
for name in gui-install.sh install.sh gateway.sh gatewayctl config.sh org.airgateway.gateway.plist; do
  [ -f "$SOURCE/$name" ] && [ ! -L "$SOURCE/$name" ] &&
    [ "$(/usr/bin/stat -f '%u:%Lp:%l' "$SOURCE/$name")" = 0:644:1 ] || fail 'Packaged code ownership or permissions changed.'
done
scratch=$(/usr/bin/mktemp -d /private/tmp/air-gateway-gui.XXXXXXXX)
