...
将保持唤醒的 Mac 作为 IPv4 网关：Mac 通过 Wi-Fi 连接上游，独立以太网接口连接路由器 WAN，下游设备使用路由器的 LAN 或 Wi-Fi。
...
> 请读取仓库的 README.md、AGENTS.md 和 air-gateway/docs/ai-setup.md，帮助我把这台 Mac 配置为 Wi-Fi 上游、有线路由器下游的共享网关。先只读检查环境，形成具体配置与恢复方案，再完成安装和真实下游验收。需要我接线、输入密码或提供权限时说明具体操作。

只有聊天能力的 AI 可以指导命令，不能自行配置电脑。管理员认证、校园认证和接线可能需要用户参与。没有浏览器控制能力或路由器管理会话时，由用户按 AI 生成的 WAN 参数表填写。

- [AI 部署手册](air-gateway/docs/ai-setup.md)：判断顺序、现有命令、失败处理。
- [安装与使用](air-gateway/README.md)：配置格式、安装、状态、停止和卸载。
- [DNS 与代理](air-gateway/docs/dns-and-proxy.md)：处理部分校园网站打不开。
- [排障指南](air-gateway/docs/troubleshooting.md)：区分上游、转发、解析和客户端问题。
- [后续实现计划](air-gateway/docs/roadmap.md)：明确区分现有能力与尚未实现的接口。
...
python3 air-gateway/tools/check-publication.py --staged
```

校验通过并提交后，可用下列命令从已提交的文件生成源码包，不遍历整个工作区：

```sh
/bin/bash air-gateway/tools/package.sh
```
